...
```{r}
rm(list=ls())
VASGX <- tq_get("IBM") |> filter(date >="2025-01-01")
VASGX_div <- tq_get("IBM",get="dividends")
VASGX|> select(date,close,adjusted) |> summary()
```
...
ggplot(VASGX) + geom_line(aes(x=date,y=close,colour="Close"),linewidth=1) +
   ...
   geom_line(aes(x=date,y=adjusted,colour="Adjusted"),linewidth=1)
  theme_tq() + labs(title=" IBM Closng Prices w/ 100 day MA")
...
VASGX %>%
    ...
    coord_x_date(xlim = c("2025-04-01", "2025-07-07"),
                 ylim = c(210, 295))
```
...
ibm_data <- tq_get("IBM", from = floor_date(Sys.Date(), "year")) %>%
  select(date, close)
...
autoplot(ets_forecast, ibm_tsibble, level = 95) +
  labs(
    title = "14-Day Forecast for IBM Closing Price (ETS)",
...
autoplot(arima_forecast, ibm_tsibble, level = 95) +
  labs(
    title = "14-Day Forecast for IBM Closing Price (ARIMA)",
...
fc %>%
  autoplot(IBM_ts) +
  labs(title = "ARIMA Forecast 14 Day (IBM)",
       y = "Close Price") +
  geom_smooth(
...
ibm_data <- tq_get("IBM", from = start_date) %>%
  select(date, close) %>%
...
ggplot(VASGX_div) + geom_point(aes(x=date,y=value)) +
  geom_smooth(aes(x=date,y=value),method="lm") +
  labs(title="IBM Dividend History")
```