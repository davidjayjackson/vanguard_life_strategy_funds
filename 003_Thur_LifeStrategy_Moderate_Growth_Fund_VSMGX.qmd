---
title: "Vanguard LifeStrategy Moderate Growth Fund(VSMGX)					"
format: html
editor: visual
---
...
The Vanguard LifeStrategy Moderate Growth Fund (VSMGX) is a mutual fund offered by Vanguard. It's designed for investors seeking a balance between capital appreciation and a low to moderate level of current income.
...
### Boxplot and Histogram of Closing Prices

```{r}
rm(list=ls())
history <- tq_get("VSMGX")|> filter(date >="2025-01-01")
daily <- history |>filter(date >="2025-01-01")
div <- tq_get("VSMGX",get="dividends")
history |> select(date,close) |> summary()
```

### Histogram

```{r}
ggplot(daily,aes(x=close)) + geom_histogram()
```

### Boxplot

```{r}
ggplot(daily,aes(x=close)) + geom_boxplot()
```

### Close + 100 Day Moving Average

```{r}
history$rolling_100d <- movavg(history$close,100,type=c("s"))
ggplot(history) + geom_line(aes(x=date,y=close,colour="Close"),linewidth=1) +
  geom_line(aes(x=date,y=rolling_100d,colour="50d Rolling"),linewidth=1) +
  theme_tq() + labs(title="VSMGX Closng Prices w/ 100 day MA (Stating w/2000-01-01)")
...
ibm_data <- tq_get("VSMGX", from = floor_date(Sys.Date(), "year")) %>%
  select(date, close)
...
fc %>%
  autoplot(IBM_ts) +
  labs(title = "ARIMA(AI ) Forecast 14 Day (VSMGX )",
       y = "Close Price") +
  geom_smooth(
...
```{r}
ggplot(div) + geom_col(aes(x=date,y=value))
```